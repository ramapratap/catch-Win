...
export const storage = {
  getSettings: (): GameSettings => {
    try {
      const stored = localStorage.getItem('mouth_catch_settings');
  ...
  saveSettings: (settings: GameSettings) => {
    try {
      localStorage.setItem('mouth_catch_settings', JSON.stringify(settings));
    } catch (error) {
      console.error('Failed to save settings:', error);
    }
  },
  getTotalCoins: (): number => {
    try {
      const stored = localStorage.getItem('mouth_catch_total_coins');
      return stored ? parseInt(stored, 10) : 0;
    } catch {
      return 0;
    }
  ...
  addCoins: (amount: number) => {
    try {
      const current = storage.getTotalCoins();
      localStorage.setItem('mouth_catch_total_coins', (current + amount).toString());
    } catch (error) {
      console.error('Failed to add coins:', error);
    }
  }
